pipeline {
    ...
    options {
        timestamps()
        ansiColor('xterm')
        skipDefaultCheckout(true) //Checkout  multibranch desde etapa GetCode.
    }
    ...
    environment {
        ...
        //Entorno Actual: Se mueve a etapa Inicializacion
        //CONFIG_ENV = 'staging'
        //env.CONFIG_ENV = "${env.BRANCH_NAME == 'master' ? 'production' : 'staging'}"
    ...
    stages {
        ...
        stage('Get Code') {
            ...
            steps {
                ...
                //Checkout multibranch sobre workspace limpio.
                checkout scm
                ...
                // Descarga la rama del repositorio: El Multibranch ya tiene la rama correcta.     
                //git branch: "${env.BRANCH_NAME}",
                //git branch: 'develop',
                //    credentialsId: 'github-pat',
                //    url: 'https://github.com/guarrior-labs/todo-list-aws.git'

                // Descarga unicamente el fichero de configuracion
        ...
        stage('Initializa') {
            ...
            steps {
                ...
                echo 'Entorno de despliegue: STAGING & PRODUCTION'
                echo '==========================================='
        ...
        /**************************************************************
        * Publica resultados de pruebas estaticas, solo en staging,
        * y despues de TESTS para evitar "condicion carrera".
        **************************************************************/
        ...
        stage('Promover') {
            ...
            steps {
                ...
                {
                sh '''
                    # Elimina la configuración descargada del repositorio de configuración.
                    rm -f samconfig.toml
